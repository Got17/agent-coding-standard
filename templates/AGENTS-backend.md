...
1. Architecture: Transport → Service → Data layering only; zero business logic in handlers; DI via interfaces; machine-checkable API contracts (OpenAPI 3.1/Protobuf/GraphQL schema/equivalent); unified 5-key error envelope (code, message, details, timestamp, request_id).
2. Security: Verified short-lived credentials/session tokens; BOLA — scope protected user/tenant resources by authenticated context unless explicitly public/system-scoped; BOPLA — explicit write allowlists, no mass assignment to ORM models; strict schema validation (reject undeclared fields → 400); secrets via env/vault only; distributed rate limiting (429 + Retry-After).
3. Data: Expand-Migrate-Contract zero-downtime migrations; 100% parameterized SQL; no N+1 (joins/eager load/DataLoader); statement_timeout caps; short TX at service layer (no network I/O inside TX); transactional outbox & consumer idempotency; READ COMMITTED minimum for relational DBs or documented equivalent consistency for non-relational stores; 4-key audit metadata (created_at, updated_at, created_by, updated_by).
4. Observability: Structured JSON logs (DEBUG/INFO/WARN/ERROR); W3C traceparent/tracestate + trace_id/correlation_id on every log + outgoing call; auto PII redaction; /healthz/liveness + /healthz/readiness probes; OpenTelemetry-compatible traces and metrics.
...
6. Testing: target mostly unit tests with focused containerized integration tests and lightweight E2E smoke; no shared mutable state; API contract diff in CI; negative authorization test per protected resource (403/404, not data).
7. Code quality: SRP + DIP; pragmatic DRY/YAGNI (no speculative abstractions); guard clauses over nesting; intent comments (why); docstrings in sync with machine-checkable API contracts.
-->
...
- [ ] **Contract-First API Design**: APIs MUST be defined using machine-checkable contracts such as OpenAPI 3.1, Protocol Buffers, GraphQL schemas, AsyncAPI, or an equivalent project-approved schema format. Breaking API changes require a major version bump or explicit sunset headers (`Sunset`, `Deprecation`) where the protocol supports them.
...
- [ ] **Authentication Baseline**: Authenticate API requests using short-lived OAuth 2.0 / OIDC JWTs, opaque access tokens, or secure server-backed sessions transported via `Authorization: Bearer` headers or secure `httpOnly`, `Secure`, `SameSite` cookies for web clients. Token signature/introspection and relevant claims (`iss`, `aud`, `exp`) MUST be verified on every protected request. Requests with missing, expired, or invalid credentials MUST be rejected immediately with `401 Unauthorized`.
- [ ] **OWASP BOLA/IDOR Prevention**: Every query for protected user-owned or tenant-owned data MUST scope access by the authenticated user/tenant ID extracted from trusted server-side auth context. Public resources, admin operations, system jobs, and backfills MUST explicitly document their access scope. Never rely solely on client-provided route IDs.
...
- [ ] **Transaction Isolation Level**: Relational database transactions MUST use at minimum `READ COMMITTED` isolation. `SERIALIZABLE` isolation MUST be used only for operations that require strict consistency guarantees. For non-relational stores, document the equivalent read/write consistency guarantees required by the operation. Default engine isolation levels MUST be explicitly confirmed and not assumed.
...
- [ ] **Testing Pyramid**: Prefer a high proportion of fast unit tests for domain business logic, focused containerized integration tests for repositories/handlers (using Testcontainers or ephemeral DBs), and lightweight E2E smoke tests for critical flows. Treat ratios such as 70% unit / 20% integration / 10% E2E as targets, not mechanical quotas.
...
- [ ] **Contract Validation in CI**: Automatically run API contract diff validation in CI pipelines to prevent unintended breaking changes across OpenAPI, Protobuf, GraphQL, AsyncAPI, or the project's chosen contract format.

...
> Universal production baseline for frontend web applications, using React ecosystem examples throughout (adapt equivalents for Vue/Svelte/Solid). Extended by framework/meta-framework templates (e.g., `AGENTS-nextjs.md`).

<!-- AI-COPY-BLOCK
NON-NEGOTIABLE FRONTEND RULES (enforce every PR):
1. Components: pure presentational vs container/hook separation; typed props only; MUST NOT exceed 200 lines or 3 unrelated state slices; named exports.
2. State: server state via async cache (TanStack Query/SWR) only; no duplication into useState; explicit mutation invalidation; no window.location.reload() — use cache invalidation APIs instead; localized error boundaries.
...
- [ ] **Single Responsibility & Line-Cap Limits**: Components MUST NOT exceed 200 lines of code or manage more than 3 unrelated pieces of state. Complex components MUST be refactored into focused sub-components or custom hooks.
...
- [ ] **LCP Asset Prioritization**: The page's primary LCP element (hero image, above-the-fold banner) MUST use `fetchpriority="high"` and `<link rel="preload">` to ensure it begins loading at the earliest browser opportunity, avoiding late discovery by the parser.
